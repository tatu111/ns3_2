        	 ...
        	 else
        	 {
        		 if(!m_socket){
                     ...
                     //send
                   	...
                   	m_socket_local->SendTo (packet, 0, InetSocketAddress (bcast, 49300));
                   	 }
...
void SimulationProposalReactive::ScheduleCorecheck ()
{
  NS_LOG_FUNCTION (this);
...
//void SimulationProposalReactive::ScheduleCorecheckEvent ()
//{
//  NS_LOG_FUNCTION (this);
//
//  m_CorecheckEvent = Simulator::Schedule (m_checkinterval, &SimulationProposalReactive::ScheduleCorecheck, this);
//}
  ...
  while (buffer->GetSize () >= header.GetSize ())
    {
      ...
      if(header.GetMessageType()==SimulationHeader::CORE_MESSAGE)
      {
    	  ...
    	  if(core.GetDestinationAddress() == ipv4->GetAddress (0, 0).GetLocal ())
    	  {
    		  core_flag = 1;
    	  }

//         m_rxTraceWithSeqTsSize2 (complete, from, localAddress, header);
//      case SimulationHeader::FLOODING_MESSAGE:
